...
|----|------|-------|--------|---------|-------|-----------|-------|
| TASK-045 | Build a comment-sync workflow for the client project reference page so Elvis can comment directly on the page and have comments sync back into the repo | Aakash | To Do | | | | Raised on the 2026-09-02 call. Either path (comment on the page, or edit the workspace) must sync to one source. Medium. Source: comms/meeting-notes/2026-09-02_Wepop_open-questions-and-repo-migration.md. |
| TASK-044 | Migrate the Wepop docs repo from Aakash's personal GitHub to the new BetaCraft GitHub repo, then send Elvis the sync steps (accept invite, re-clone, update origin, re-point the Cowork folder) | Aakash | To Do | | | | Agreed 2026-09-02. Elvis has pushed his latest and is frozen until Aakash confirms the migration is done. Deepak already shared the Git remote URLs. High. Source: comms/meeting-notes/2026-09-02_Wepop_open-questions-and-repo-migration.md. |
| TASK-043 | Korean-label review of the categories taxonomy v2.0 (all EN/KO node pairs) before launch | Korean-label reviewer | To Do | | | | Taxonomy ships paired EN/KO labels under one canonical ID; needs a native review pass. Open item TX-2. Medium. Source: DEC-051; workspaces/elvis/categories-taxonomy-2026-08-27.md. |
| TASK-042 | Add gambling to the moderation blocklist so gambling/betting content is caught after Casino and poker night was removed from the taxonomy for 도박죄 exposure | Elvis (mod), Aakash (compliance) | To Do | | | | Compliance action riding on DEC-051. Removing the category is not enough; the blocklist enforces it. Ties to the moderation launch blocker (TASK-034). Open item TX-5. High. Source: DEC-051; workspaces/elvis/categories-taxonomy-2026-08-27.md. |
| TASK-041 | Adopt the handoff spec's I-N invariant numbering into CLAUDE.md as a maintained registry, re-scoping I-12 to distinguish participant rating from host rating | Aakash | To Do | | | | Handoff spec cites I-6..I-20 but no registry exists here; CLAUDE.md section 8 is an unnumbered list. I-12 as drafted forbids host ratings, contradicting DEC-014; replacement wording in handoff-spec-v0.9-intake-2026-08-29.md item A. Fix the stale phone-OTP invariant (predates DEC-011) in the same pass. Medium. Source: handoff spec section 13. |
| TASK-040 | Route the handoff spec legal register (L-1..L-12) to DLG Law as a single consult, L-3 treated as P0 blocker, L-8 folded into TASK-013 | Aakash | To Do | | | | Twelve-item register never routed. L-3 (위치정보법 registration for the geofenced check-in) is BLOCKING before P0, distinct from TASK-013. L-8 (PIPA 만 14세 미만 guardian consent) folds into the TASK-013 age/location consult. Companion HOTSHEET Blocking entry + risk R5. High. Source: handoff spec section 16. |
| TASK-039 | Write the CSAM preserve-and-report runbook (one page, any reviewer can follow unaided) and have DLG Law review it before launch | Elvis (draft), Aakash (DLG review) | To Do | | | | Hard pre-launch legal gate not covered by TASK-034's tooling/SLA scope. The intuitive delete-it response is legally wrong: preserve, restrict access, report. Handoff §12.5 + legal register L-12 (불법촬영물 under 전기통신사업법). Companion HOTSHEET Blocking entry. High. Source: handoff-spec-v0.9-intake-2026-08-29.md. |
...
| TASK-036 | Scope the payments/ticketing build (Stripe Connect style splitting, host KYC, refunds, tax) and decide whether it is phase 1 | Aakash | To Do | | | | DEC-010, DEC-018. Flagged largest technical scope; own conversation. Korea note (2026-08-26 sync): Stripe not usable in Korea, needs a Korean gateway or card path; app-store IAP fees 15-30 percent with a virtual-goods vs physical-experience distinction; Aakash can source a cheaper India-style gateway. |
| TASK-035 | Confirm the cohort transitions and the cross-enrollment dual-access case with Elvis | Aakash | To Do | | | | DEC-019, DEC-020. Unconfirmed: (1) the cohort filter softens back to a ranking signal once a city is dense enough; (2) who owns the manual per-city density call; (3) cross-enrollment - a user who joins general then enrolls as a student. University affiliation overrides, so they move into the (city, university-affiliated) cohort and, under the launch hard filter, are restricted to it (not both data sets), but Elvis's doc does not spell out the transition and parks 'can a user override their own cohort'. |
...
| TASK-033 | Design a general user-blocking feature and an attendee-level (thumbs) feedback mechanism | Elvis (+ Deepak) | Done | | 2026-08-31 | | Design delivered by DEC-037 (general blocking: bidirectional, total, phase-1 baseline) and DEC-036 (feedback resolved). Scope shift: thumbs up/down replaced by a single positive-only tap (DEC-034), so the original thumbs mechanism was dropped, not built. Build now tracked as phase-1 scope-matrix rows (general user-blocking; attendee positive-only tap). |
...
| TASK-028 | Confirm the Event data model (multi-day date range) and other schema prerequisites with Deepak | Deepak | Done | | 2026-08-31 | | Multi-day date range confirmed by DEC-041 (Event supports start/end via scheduled_end, Airbnb-style picker). Remaining per-stop-date and calendar-picker schema flags (DEC-041) and the DEC-043/044 accountability schema flags are tracked on the individual build tasks. DEC-025/021/022. |
...
| TASK-017 | Build Elvis's client-specific input skill so he feeds info in the structure the dev harness expects | Aakash | Done | 2026-08-19 | 2026-08-26 | | From the 2026-08-19 Elvis setup call. Marked complete by Aakash 2026-08-26. |
...
| TASK-010 | Elvis sends the reviewed project documentation | Elvis | Done | | 2026-08-26 | | Ref todos #1. Elvis's design docs shared and processed via the 2026-08-26 intake (DEC-010 to DEC-025). Marked complete by Aakash 2026-08-26. |
